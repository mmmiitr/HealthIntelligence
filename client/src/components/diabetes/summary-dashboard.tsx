...
export default function SummaryDashboard({ timeFilter, viewMode, showForecast, onNavigateToFinance }: SummaryDashboardProps) {
  ...
  const handleCardClick = () => {
    onNavigateToFinance(); // Call the prop function
  };
      ...
      <DashboardSection>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          {keyMetrics.slice(0, 3).map((metric) => (
            // Apply onClick and cursor-pointer to a wrapper div for the first three cards
            <div
              key={metric.title}
              onClick={handleCardClick}
              className="cursor-pointer transition-all duration-300 hover:shadow-lg" // Added hover effect
            >
              <StandardMetricCard
                title={metric.title}
